...
detector = dlib.cnn_face_detection_model_v1("mmod_human_face_detector.dat")  # Changed to MMOD
...
def detect_face_and_eyes(image, face_rect=None):
    ...
    if face_rect is None:
        faces = detector(gray, 1)  # Upsample 1 time for better accuracy
        if len(faces) == 0:
            return None
        # MMOD returns mmod_rect, extract rectangle and sort
        faces = sorted(faces, key=lambda x: x.rect.width() * x.rect.height(), reverse=True)
        face = faces[0].rect  # Extract rectangle from mmod_rect
    else:
        face = face_rect
...
def compute_transformation_matrix(frame, face_rect, target_eye_ratio=0.2):
...
def process_videos(video_dir, output_dir, debug_dir, failed_dir="failed_videos"):
    ...
    for video in sorted(os.listdir(video_dir)):
        ...
        # Process faces, extract rectangle from mmod_rect
        faces = sorted(faces, key=lambda x: x.rect.width() * x.rect.height(), reverse=True)
        faces_to_process = [f.rect for f in faces[:min(3, len(faces))]]  # Extract rectangles
        processed_count = 0
