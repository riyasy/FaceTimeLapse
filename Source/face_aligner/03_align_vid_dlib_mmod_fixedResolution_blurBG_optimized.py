...
def process_video(video_path, output_path, debug_dir, face_rect=None, face_suffix=""):
    ...
    if M is None or M2 is None:
        print(
            f"No face detected in first frame of {video_path} for {face_suffix}, skipping."
        )
        cap.release()
        return False
...
def process_videos(video_dir, output_dir, debug_dir, failed_dir="failed_videos"):
    ...
    for video in sorted(os.listdir(video_dir)):
        ...
        # Scale down the first frame for initial detection (1/2 size)
        scale_factor = 0.5
        scaled_frame = cv2.resize(
            first_frame,
            (
                int(first_frame.shape[1] * scale_factor),
        ...
        # Scale up face coordinates to match original frame
        scale_up_factor = 1 / scale_factor  # e.g., 2 for scale_factor = 0.5
        faces = sorted(
            faces, key=lambda x: x.rect.width() * x.rect.height(), reverse=True
        )
        faces_to_process = [
        ...
        for i, face in enumerate(faces_to_process, 1):
            ...
            success = process_video(
                video_path, output_path, debug_dir, face, face_suffix
            )
            if success:
                processed_count += 1

        processing_results[video_path] = {
            "detected": len(faces),
...
# Run processing
process_videos(
    "02_input_videos_cfr", "08_output_videos", "99_debug_frames", "92_failed_videos"
)
