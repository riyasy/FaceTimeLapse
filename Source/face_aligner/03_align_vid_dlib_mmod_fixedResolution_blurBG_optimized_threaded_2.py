...
def process_single_video(video_path, output_dir, debug_dir, failed_dir):
    """Processes a single video and returns result."""
    ...
    for i, face in enumerate(faces_to_process, 1):
        ...
        if success:
            processed_count += 1
    result = {"detected": len(faces), "processed": processed_count}
...
def process_videos(video_dir, output_dir, debug_dir, failed_dir="failed_videos"):
    """Processes all videos in the folder with multithreading."""
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_video = {
            executor.submit(
                process_single_video, video_path, output_dir, debug_dir, failed_dir
            ): video_path
